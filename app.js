...
function renderTask17() {
    ...
    // Recalc 1-6
    const sortedX = [...X].sort((a, b) => a - b);
    const k_new = Math.floor(1 + 3.322 * Math.log10(n));
    const minX = sortedX[0];
    const maxX = sortedX[n - 1];
    ...
    if (state.Z && state.Z.length === n && state.Y && state.Y.length === n) {
        pcaText = "Виконано Метод Головних Компонент (PCA) для вибірок X, Y, Z. Матриця коваріації була розкладена на власні вектори та власні значення. Відповідно до часток поясненої дисперсії кожною компонентою розроблено рекомендації щодо зниження розмірності. Дані спроектовано на площину перших двох головних компонент.";
    ...
    for (let i = 0; i < n; i++) {
        dataTableRows += `
            <tr>
                ...
                <td style="border:1px solid #000; padding:4px;">${X[i].toFixed(4)}</td>
                <td style="border:1px solid #000; padding:4px;">${state.Y && state.Y[i] ? state.Y[i].toFixed(4) : '-'}</td>
                <td style="border:1px solid #000; padding:4px;">${state.Z && state.Z[i] ? state.Z[i].toFixed(4) : '-'}</td>
            </tr>
        `;
    }

    const reportHtml = `
        <h1 style="text-align: center; font-size: 20px; margin-bottom: 20px;">ПІДСУМКОВИЙ ЗВІТ<br>з розрахунково-графічної роботи</h1>
        <p><strong>Дата формування:</strong> ${dateStr}</p>
        <p><strong>Обсяг досліджуваної вибірки (n):</strong> ${n}</p>
        
        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Вхідні дані (Вибірка)</h2>
        <p>Для розрахунково-графічної роботи було використано такий масив випадкових чисел:</p>
        <table style="width:100%; border-collapse: collapse; text-align: center; font-size: 12px; margin-bottom: 15px;">
            <thead>
                <tr style="background-color: #f0f0f0;">
                    <th style="border:1px solid #000; padding:4px;">№</th>
                    <th style="border:1px solid #000; padding:4px;">Ознака X</th>
                    <th style="border:1px solid #000; padding:4px;">Ознака Y</th>
                    <th style="border:1px solid #000; padding:4px;">Ознака Z</th>
                </tr>
            </thead>
            <tbody>
                ${dataTableRows}
        ...
        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 1: Побудова варіаційного ряду</h2>
        <p>Отриману вибірку було впорядковано за зростанням від найменшого елемента (X_min = ${round(Math.min(...X))}) до найбільшого (X_max = ${round(Math.max(...X))}). Впорядкування (ранжування) елементів є базовим етапом статистичного аналізу.</p>

        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 2: Метод згрупованих даних</h2>
        <p>Варіаційний ряд було розбито на інтервали (класи) однакової ширини. Кількість інтервалів k визначено за формулою Стерджеса: <i>k = 1 + 3.322 * lg(n)</i>. Ширина інтервалу h обчислена як <i>h = (X_max - X_min) / k</i> з округленням у більший бік для уникнення втрати крайніх значень.</p>

        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 3 та 4: Побудова графіків</h2>
        <ul style="margin-left: 20px; margin-bottom: 15px;">
            <li><b>Гістограма:</b> Побудована за інтервальним рядом, де висота кожного стовпця відповідає абсолютній частоті попадання значень в інтервал.</li>
            <li><b>Полігон частот:</b> Отримана ламана лінія, що з'єднує точки (середина_інтервалу; частота).</li>
            <li><b>Емпірична функція розподілу F*(x):</b> Побудована класичним способом у вигляді ступінчастої функції, що здійснює стрибок розміром 1/n в кожній точці варіаційного ряду.</li>
        </ul>
        <div id="report-plot-histogram" style="width:100%; height:300px; margin-bottom: 20px;"></div>
        <div id="report-plot-edf" style="width:100%; height:300px; margin-bottom: 20px;"></div>

        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 5 та 6: Описові (числові) статистики</h2>
        <p>Ми обчислили ключові показники центру та розкиду розподілу:</p>
        <ul style="margin-left: 20px; margin-bottom: 15px;">
            <li><b>Середнє значення (x̄):</b> ${mean}. Використовувалась формула математичного сподівання вибірки: <i>x̄ = (Σ x_i) / n</i></li>
            <li><b>Дисперсія (S²):</b> ${variance}. Обчислена як незсунена оцінка (ділення на n-1): <i>S² = Σ (x_i - x̄)² / (n - 1)</i></li>
            <li><b>Середнє квадратичне відхилення (S):</b> ${sd}. Обчислено як корінь з дисперсії: <i>S = √S²</i></li>
            <li><b>Мода інтервального ряду (Mo):</b> Визначена як середина найчастішого інтервалу або за класичною формулою Гаусса: <i>Mo = X_start + h * (n_Mo - n_prev) / ((n_Mo - n_prev) + (n_Mo - n_next))</i></li>
            <li><b>Асиметрія:</b> ${skewness}. Використовується для визначення ступеня "скошеності" графіка відносно його центру.</li>
        </ul>

        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 7: Виявлення грубих помилок (викидів)</h2>
        <p>Для перевірки вибірки на наявність аномальних значень було застосовано метод міжквартильного розмаху (IQR). Обчислено перший Q1 та третій Q3 квартилі. Нижня межа: Q1 - 1.5*IQR. Верхня межа: Q3 + 1.5*IQR. Усі значення за цими межами вважаються грубими помилками (викидами) і можуть підлягати вилученню.</p>

        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 8 та 9: Перевірка статистичних гіпотез про нормальний розподіл</h2>
        <ul style="margin-left: 20px; margin-bottom: 15px;">
            <li><b>Завдання 8 (Критерій Колмогорова):</b> Ми знайшли максимальне абсолютне відхилення D між емпіричною функцією F*(x) (у точках стрибка) та теоретичною нормальною F(x). Обчислено статистику Колмогорова λ = D * √n та порівняно з критичним значенням 1.36.</li>
            <li><b>Завдання 9 (Критерій Пірсона χ²):</b> Емпіричні частоти кожного інтервалу були порівняні з очікуваними (теоретичними) ймовірностями потрапляння випадкової величини в ці інтервали згідно із нормальним законом. Межі крайніх інтервалів було розширено до ±∞, а інтервали з очікуваною частотою &lt; 5 — об'єднано для дотримання суворих статистичних умов перевірки.</li>
        </ul>

        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 10: Побудова довірчого інтервалу</h2>
        <p>Побудовано довірчий інтервал для математичного сподівання з рівнем надійності γ = 0.95. Оскільки дисперсія генеральної сукупності невідома, використано розподіл Ст'юдента (t-критерій) з n-1 ступенями вільності. Межі обчислено за формулою: <i>x̄ ± t_crit * (S / √n)</i>.</p>

        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 11-15: Кореляційний та Регресійний аналіз, ANOVA</h2>
        <ul style="margin-left: 20px; margin-bottom: 15px;">
            <li><b>Коефіцієнт кореляції Пірсона (r):</b> ${r}. Формула: <i>r = cov(X,Y) / (S_x * S_y)</i>. Визначає силу лінійного зв'язку між двома змінними X та Y.</li>
            <li><b>Завдання 11, 13 (Лінійна регресія):</b> Побудовано модель Y(X) методом найменших квадратів (МНК), що мінімізує суму квадратів відхилень емпіричних точок від лінії регресії. Отримане рівняння: <b>${regEq}</b></li>
            <li><b>Завдання 14 (Нелінійна регресія):</b> Знаходження коефіцієнтів для поліноміальної регресії (параболи) Y = aX² + bX + c також шляхом розв'язання системи лінійних алгебраїчних рівнянь за МНК.</li>
            <li><b>Завдання 15 (ANOVA):</b> Проведено Дисперсійний аналіз регресії (Regression ANOVA). Отримано значення F-статистики Фішера, яке порівнюється з F-критичним для перевірки статистичної значущості побудованого лінійного рівняння регресії.</li>
        </ul>
        <div id="report-plot-reg" style="width:100%; height:300px; margin-bottom: 20px;"></div>

        <h2 style="font-size: 16px; margin-top: 15px; border-bottom: 1px solid #ccc; padding-bottom: 5px;">Завдання 16: Багатовимірний аналіз (PCA)</h2>
        <p>${pcaText}</p>
        <div id="report-plot-pca" style="width:100%; height:300px; margin-bottom: 20px;"></div>
        
        <br>
        <p style="text-align: right; margin-top: 40px; margin-bottom: 20px;"><b>Згенеровано автоматично в системі StatApp</b></p>
    `;
    ...
    // Render plots
    const layoutOpt = {
        paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)', font: { color: '#000' },
        margin: { t: 10, r: 10, l: 40, b: 20 }
    };
    
    Plotly.newPlot('report-plot-histogram', [traceHist, tracePoly], layoutOpt);
    Plotly.newPlot('report-plot-edf', [traceEDF], layoutOpt);
    
    if (traceScatter && traceLinReg && traceNonLinReg) {
        Plotly.newPlot('report-plot-reg', [traceScatter, traceLinReg, traceNonLinReg], layoutOpt);
    }
    if (tracePCA) {
        Plotly.newPlot('report-plot-pca', [tracePCA], layoutOpt);
    }
}
